...
namespace VballManager
{
    ...
    public class VolleyballClub
    {
       ...
       public int MaxDropinFeeOwe
        ...
        public List<Pool> FindFridayPools()
        {
            return Pools.FindAll(p => p.DayOfWeek == DayOfWeek.Friday);
        }
       ...
       public void AddReservationNotifyWechatMessage(String playerId, String operatorId, String result, Pool targetPool, Pool originalPool, DateTime gameDate)
        ...
        public Game FindFridayGameInSameWeek(Pool fridayPool, DateTime mondayGameDate)
        {
            Game fridayGame = fridayPool.Games.OrderBy(game => game.Date).ToList<Game>().Find(game => game.Date > mondayGameDate);
            if (fridayGame == null || fridayGame.Date.AddDays(-7) > mondayGameDate.Date) return null;
            return fridayGame;
        }
